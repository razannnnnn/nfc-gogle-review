...
export async function createCard(id, initialData = {}) {
  await connectToDatabase();
  const cardData = {
    id,
    status: initialData.status || "belum_aktif",
    nama_toko: initialData.nama_toko || null,
    link_google_review: initialData.link_google_review || null,
    link_google_review_asli: initialData.link_google_review_asli || null,
    owner_kontak: initialData.owner_kontak || null,
    dibuat_pada: new Date().toISOString(),
    diaktivasi_pada: initialData.status === "aktif" ? new Date().toISOString() : null,
    jumlah_scan: 0,
    jumlah_scan_nfc: 0,
    jumlah_scan_qr: 0,
  };
...
export async function deleteCard(id) {
  await connectToDatabase();
  const deleted = await Card.findOneAndDelete({ id }).lean();
  return cleanDocument(deleted);
}
